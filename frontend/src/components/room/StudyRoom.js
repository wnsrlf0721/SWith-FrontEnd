...
const StudyRoom = ({ location }) => {
  ...
  const createPeerConnection = (socketID, nickName) => {
    ...
    newPC.ontrack = (e) => {
      ...
      setConnnectedUsers(oldUsers => [...oldUsers, {
        id: socketID,
        nickName: nickName,
        stream: e.streams[0]
      }]);
  ...
  const startSharingScreen = () => {
    navigator.mediaDevices.getDisplayMedia({
      ...
      stream.getVideoTracks()[0].addEventListener('ended', () => {
        ...
        if (senderMediaStream && userVideoRef.current)
          userVideoRef.current.srcObject = senderMediaStream;
  ...
  const stopSharingScreen = () => {
    if (senderScreenStream)
      senderScreenStream.getTracks().forEach(track => track.stop());
    else
      return;
    if (userVideoRef.current)
      userVideoRef.current.srcObject = senderMediaStream;
